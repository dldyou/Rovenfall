...
career, four active skill slots, skill cooldown deadlines, and a bounded
...
`RpgPlayerStateService` is the server-thread mutation boundary. Activity XP also
advances the active career and awards one skill point per newly reached career
level. Promotion, skill learning, active-slot assignment, and cooldown starts
validate the current immutable RPG definition snapshot before replacing the
player record. The `Snapshot` view is immutable and suitable for administration
queries or background read-only work. `RpgPlayerSnapshotStore` can persist an
atomic compressed NBT copy under `rovenfall/snapshots/rpg`; it refuses overwrite,
size violations, malformed data, and unsupported schemas.
