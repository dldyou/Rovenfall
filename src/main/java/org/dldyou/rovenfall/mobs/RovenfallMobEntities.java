...
public final class RovenfallMobEntities {
    ...
    public static final Identifier RUNE_SENTINEL_ID = id("rune_sentinel");
    ...
    public static final DeferredHolder<EntityType<?>, EntityType<RuneSentinel>> RUNE_SENTINEL =
            ENTITY_TYPES.registerEntityType("rune_sentinel", RuneSentinel::new, MobCategory.MONSTER,
    ...
    static Identifier definitionId(EntityType<?> type) {
        ...
        if (GROVE_STALKER_ID.equals(typeId) || OREBOUND_BEETLE_ID.equals(typeId)
                || RUNE_SENTINEL_ID.equals(typeId)) {
            return typeId;
        }
        return null;
    }
    ...
    private static void createAttributes(EntityAttributeCreationEvent event) {
        ...
        event.put(RUNE_SENTINEL.get(), Zombie.createAttributes().build());
    }

    private static void registerSpawnPlacements(RegisterSpawnPlacementsEvent event) {
        event.register(RUNE_SENTINEL.get(), SpawnPlacementTypes.ON_GROUND,
                Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
                (type, level, reason, pos, random) -> Monster.checkMonsterSpawnRules(type, level, reason, pos, random)
                        && RovenfallMobRuntime.canSpawnNaturally(level.getLevel(), RUNE_SENTINEL_ID, pos, random),
