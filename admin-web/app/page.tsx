...
function readConsoleUrl(search: string): ConsoleUrlState {
  ...
  const value = (key: string, maximum = 128) =>
    (params.get(key) ?? '').trim().slice(0, maximum);
  ...
  return {
    ...
    auditFilters: {
      player: value('player'),
      action: value('action'),
      outcome: ['success', 'denied', 'failed', 'no_change'].includes(outcome)
        ? outcome
        : '',
      from: date('from'),
      to: date('to'),
...
export default function Home() {
  ...
  if (!token) {
    return (
      <Login locale={locale} setLocale={setLocale} t={t} onConnect={connect} />
    );
  }
  return (
    <Console
  ...
  async function submit(event: React.SyntheticEvent<HTMLFormElement>) {
    ...
    if (value.length < 32) {
      setError(
        locale === 'ko'
          ? '유효한 관리 토큰을 입력하세요.'
          : 'Enter a valid administration token.',
      );
      return;
    }
    setBusy(true);
    ...
    <main className="login-page">
      ...
      <section className="login-panel block-panel">
        ...
        <span className="eyebrow">
          <ShieldCheck /> ROVENFALL · {t.localOnly}
        </span>
        <h1>{t.loginTitle}</h1>
        ...
        <form onSubmit={submit} className="login-form">
          ...
          <button
            className="pixel-button primary wide"
            disabled={busy}
            type="submit"
          >
...
function Console({
  ...
  useEffect(
    () => registerReadOnlyTools(token, handleFailure),
    [token, handleFailure],
  );
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (active === 'dashboard') {
        const value = await apiRequest<DashboardData>(
          token,
          '/api/v1/dashboard',
        );
        setDashboard(value);
        ...
        const value = await apiRequest<PageData<AuditRow>>(
          token,
          `/api/v1/audit?${queryString({
            ...
            from: auditFilters.from
              ...
              : undefined,
            to: auditFilters.to
              ? new Date(auditFilters.to).getTime()
              : undefined,
            page,
            pageSize: 25,
          })}`,
        ...
        const value = await apiRequest<PageData>(
          token,
        ...
        <header className="topbar">
          ...
          <div className="search-field">
            ...
            <span className="search-scope" aria-hidden="true">
              {active === 'dashboard' ? t.allRecords : t[active]}
            </span>
            <input
              ref={searchRef}
              value={queryDraft}
            ...
            />
            {queryDraft && (
              <button onClick={() => setQueryDraft('')} aria-label="Clear">
                <X />
              </button>
            )}
          </div>
          ...
          <section className="page-heading">
            ...
            <div className="heading-actions">
              ...
              <button
                ...
                onClick={() => setActionDraft({ ...emptyAction })}
              >
      ...
      {toast && (
        <output className="toast">
          ...
          {toastTransaction && (
            <button
              onClick={() => {
                setQueryDraft(toastTransaction);
                switchView('audit');
                setToast('');
                setToastTransaction('');
              }}
      ...
      <nav aria-label="Administration">
        {navigation.map(({ id, icon: Icon }, index) => (
          ...
          >
            ...
            <span>{t[id]}</span>
            <kbd>Alt {index + 1}</kbd>
            {active === id && <ChevronRight className="nav-arrow" />}
          </button>
        ))}
      </nav>
  ...
  const metrics = [
    {
      ...
      value: dashboard.onlinePlayers,
      detail: `${t.total} ${dashboard.knownPlayers}`,
      icon: Users,
      tone: 'emerald',
    ...
    },
    {
      ...
      detail: `${formatNumber(dashboard.recentVolume, locale)} G`,
      icon: Coins,
      tone: 'gold',
    ...
    },
    {
      ...
      detail: `${t.activeShops} ${dashboard.shops}`,
      icon: LandPlot,
      tone: 'spruce',
      target: 'claims' as const,
    },
    {
      label: t.warnings,
      value: dashboard.recentAlerts,
      detail: `${t.denied} ${dashboard.recentDenied}`,
      icon: AlertTriangle,
      tone: 'redstone',
    ...
    <div className="dashboard-stack">
      <section className="metric-grid">
        {metrics.map(({ label, value, detail, icon: Icon, tone, target }) => (
          <button
            className={`metric-card block-panel ${tone}`}
            key={label}
          ...
          >
            ...
            <small>{detail}</small>
            <ChevronRight className="metric-arrow" />
          </button>
        ))}
      </section>
      <section className="dashboard-grid">
        <div className="block-panel data-panel">
          <PanelTitle
            icon={FileClock}
            title={t.recentAudit}
            detail={formatDate(dashboard.generatedAt, locale)}
          />
          <AuditTable
        ...
        <aside className="status-stack">
          <div
            className={`block-panel status-card ${dashboard.writable ? 'healthy' : 'danger'}`}
          >
            <ShieldCheck />
            <div>
              <span>{t.status}</span>
              <strong>
                {dashboard.writable ? t.serverWritable : t.serverReadOnly}
              </strong>
            </div>
          </div>
          <div className="block-panel world-card">
            <span className="eyebrow">
              <CircleDollarSign /> {t.recentVolume}
            </span>
            <strong>{formatNumber(dashboard.recentVolume, locale)} G</strong>
            <div className="ore-line">
              <i />
              <i />
              <i />
              <i />
...
function AuditFilters(props: {
  ...
  return (
    <form
      className="filter-bar block-panel"
      onSubmit={(event) => {
        event.preventDefault();
        props.apply(props.draft);
      }}
    >
      <div className="filter-presets">
        <strong>{t.quickFilters}</strong>
        <button type="button" onClick={recentDenied}>
          <FileClock />
          {t.recentDenied}
        </button>
        <button type="button" onClick={() => props.apply(emptyAuditFilters)}>
          <X />
          {t.clearFilters}
        </button>
        <span>
          {t.activeFilters}: {props.activeCount}
        </span>
      </div>
      <label>
        {t.playerFilter}
        <input
          value={props.draft.player}
      ...
      <label>
        ...
        >
          ...
          <option value="denied">{t.deniedResult}</option>
          <option value="failed">{t.failed}</option>
          <option value="no_change">{t.noChange}</option>
        </select>
      </label>
      <label>
        {t.from}
...
function DataView({
  ...
  return (
    <section className="block-panel data-panel full">
      <PanelTitle
        icon={navigation.find((item) => item.id === active)?.icon ?? FileClock}
        title={t[active]}
        detail={
          data ? `${t.total} ${formatNumber(data.totalEntries, locale)}` : ''
        }
      />
      {loading ? (
        ...
        <PlayerTable
          rows={data.entries as PlayerRow[]}
          locale={locale}
          t={t}
          onPlayer={onPlayer}
        />
        ...
        <TransactionTable
          rows={data.entries as TransactionRow[]}
          locale={locale}
          t={t}
          onPlayer={onPlayer}
          onReverse={onReverse}
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.playerId}>
              ...
              <td className="number gold-text">
                {formatNumber(row.balance, locale)} G
              </td>
              <td>{humanize(row.activeCareer || '—')}</td>
              <td className="number">{row.claims}</td>
              <td>{formatDate(row.lastSeen, locale)}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>{t.actor}</th>
            <th>{t.target}</th>
            {!compact && <th>{t.reason}</th>}
            <th>{t.result}</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row) => (
            <tr key={`${row.timestamp}-${row.transactionId}`}>
              <td className="time-cell">{formatDate(row.timestamp, locale)}</td>
              <td>
                <code>{humanize(row.action)}</code>
              </td>
              <td>
                <PlayerLink
                  id={row.actorId}
              ...
              <td className="truncate-cell" title={row.target}>
                {row.target}
              </td>
              {!compact && (
                <td className="truncate-cell" title={row.reason}>
                  {row.reason}
                </td>
              )}
              <td>
            ...
            <tr key={row.transactionId}>
              ...
              <td>
                <button
                  ...
                  onClick={() => onPlayer(row.playerId)}
                >
                  {row.playerName || shortId(row.playerId)}
                </button>
              </td>
              <td className="number gold-text">
                {formatNumber(row.amount, locale)} G
              </td>
              <td>
                <code title={row.transactionId}>
                  {shortId(row.transactionId)}
                </code>
              </td>
              <td>
                {row.reversedBy ? (
                  <span className="status-badge neutral">reversed</span>
                ) : (
              ...
              <td>
                {onReverse && (
                  <button
                    ...
                    disabled={Boolean(row.reversedBy)}
            ...
            <tr key={row.key}>
              <td>
                ...
                <small className="subcell">
                  chunk {row.chunkX}, {row.chunkZ}
                </small>
              </td>
              <td>
                <button
                  className="table-link"
                  onClick={() => onPlayer(row.ownerId)}
                >
                  {row.ownerName || shortId(row.ownerId)}
                </button>
              </td>
              <td className="number gold-text">
                {formatNumber(row.purchasePrice, locale)} G
              </td>
              <td className="number">{row.trustedPlayers}</td>
              <td>
                {row.pendingTransferTo ? (
                  <span className="status-badge warning">transfer</span>
                ) : (
                  <span className="status-badge success">active</span>
                )}
              </td>
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.shopId}>
              ...
              <td>
                {row.dimension || '—'}
                <small className="subcell">{row.position}</small>
              </td>
              <td className="number">{row.offers}</td>
              <td className="number">{row.maxDistance}m</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function Pagination({
  ...
  return (
    <footer className="pagination">
      <span>
        {t.total} {data.totalEntries} · {t.page}{' '}
        {data.totalPages ? data.page + 1 : 0}/{data.totalPages}
      </span>
      <div>
...
function PlayerDialog({
  ...
  return (
    <Modal title={t.details} closeLabel={t.close} onClose={onClose} wide>
      {error ? (
        ...
        <div className="player-detail">
          <div className="player-hero">
            ...
            <div>
              <span
                className={`status-badge ${data.online ? 'success' : 'neutral'}`}
              >
                {data.online ? t.online : t.offline}
              </span>
              <h2>{data.name || shortId(data.playerId)}</h2>
              <button
                className="copy-line"
                onClick={() => {
                  void navigator.clipboard.writeText(data.playerId);
                  setCopied(true);
                }}
              >
                <code>{data.playerId}</code>
                <Clipboard /> {copied ? t.copied : t.copyId}
              </button>
            </div>
            <div className="player-actions">
              <button
                className="pixel-button"
                onClick={() => onAudit(data.playerId)}
              >
                <FileClock />
                {t.viewPlayerAudit}
              </button>
              <button
                className="pixel-button primary"
                onClick={() => onManage(data.playerId)}
              >
                <UserRoundCog />
                {t.manage}
              </button>
          ...
          <div className="detail-grid">
            <Detail label={t.role} value={humanize(data.role)} />
            <Detail
              label={t.balance}
              value={`${formatNumber(data.balance, locale)} G`}
              gold
            />
            <Detail
              label={t.career}
              value={humanize(data.activeCareer || '—')}
            />
            <Detail label={t.claimCount} value={String(data.claimCount)} />
            <Detail
              label="XP"
              value={formatNumber(data.activityExperience, locale)}
            />
            <Detail
              label={t.lastSeen}
              value={formatDate(data.lastSeen, locale)}
            />
          </div>
          <h3>{t.progression}</h3>
          {data.challengeProgress.available ? (
            <div className="challenge-summary" aria-label={t.challenges}>
              <div>
                <span>{t.challenges}</span>
                <strong>{data.challengeProgress.total}</strong>
              </div>
              <div className="claimable">
                <span>{t.claimableChallenges}</span>
                <strong>{data.challengeProgress.claimable}</strong>
              </div>
              <div className="completed">
                <span>{t.completedChallenges}</span>
                <strong>{data.challengeProgress.completed}</strong>
              </div>
            </div>
          ) : (
            <p className="progress-unavailable">{t.progressUnavailable}</p>
          )}
          <h4 className="activity-progress-title">{t.activityProgress}</h4>
          <div className="activity-progress-grid">
            {data.activityProgress.map((row) => {
              const label = activityLabel(row.track, t);
              const percent = activityPercent(row);
              return (
                <article className="activity-progress-card" key={row.track}>
                  <header>
                    <strong>{label}</strong>
                    <span>
                      Lv. {row.level}/{row.maximumLevel}
                    </span>
                  </header>
                  <progress
                    className="activity-progress-bar"
                    aria-label={`${label} ${t.levelLabel}`}
                    max={100}
                    value={percent}
                  />
                  <footer>
                    <span>
                      {row.maximum
                        ? t.maximumLevel
                        : `${formatNumber(row.experienceIntoLevel, locale)} / ${formatNumber(row.experienceForNextLevel, locale)} XP`}
                    </span>
                    <span>{formatNumber(row.totalExperience, locale)} XP</span>
                  </footer>
                </article>
              );
            })}
...
function ActionDialog({
  ...
  token: string;
  draft: ActionDraft;
  setDraft: (value: ActionDraft | null) => void;
  t: Translator;
  locale: Locale;
  onClose: () => void;
  ...
  const update = (field: keyof ActionDraft, value: string) =>
  ...
  function payload() {
    ...
    if (draft.type === 'set_role')
      Object.assign(value, {
        playerId: draft.playerId.trim(),
        role: draft.role,
      });
    if (draft.type === 'grant_balance' || draft.type === 'debit_balance')
      Object.assign(value, {
        playerId: draft.playerId.trim(),
    ...
    if (draft.type === 'reverse')
      Object.assign(value, {
        originalTransactionId: draft.originalTransactionId.trim(),
  ...
  async function submitPreview(event: React.SyntheticEvent<HTMLFormElement>) {
    ...
    setError('');
    try {
      const result = await apiRequest<ActionPreview>(
        token,
  ...
  async function confirm() {
    ...
    setError('');
    try {
      const result = await apiRequest<OperationResult>(
        token,
        '/api/v1/actions/confirm',
        {
          method: 'POST',
          body: JSON.stringify({ previewId: preview.previewId, confirmation }),
        },
      );
      onComplete(result);
    } catch (reason) {
      ...
      if (
        ...
        ['PREVIEW_NOT_FOUND', 'PREVIEW_EXPIRED', 'STALE_PREVIEW'].includes(
          reason.code,
        )
      )
        setPreview(null);
  ...
  return (
    <Modal
      title={preview ? t.operationPreview : t.manage}
      closeLabel={t.close}
      onClose={onClose}
      ...
      {preview && details ? (
        <div className="action-review">
          ...
          <dl className="review-grid">
            <div>
              <dt>{t.selectAction}</dt>
              <dd>{humanize(String(details.type))}</dd>
            </div>
            <div>
              <dt>{t.target}</dt>
              <dd>{String(target)}</dd>
            ...
            <div>
              <dt>{t.beforeValue}</dt>
              <dd>{String(details.beforeValue ?? '—')}</dd>
            </div>
            <div>
              <dt>{t.afterValue}</dt>
              <dd>{String(details.afterValue ?? '—')}</dd>
            </div>
            <div className="wide">
              <dt>{t.reason}</dt>
            ...
            <div className="wide">
              <dt>{t.generatedTransaction}</dt>
              <dd>
                <code>{preview.transactionId}</code>
              </dd>
            </div>
            <div className={details.onlineRequired ? undefined : 'wide'}>
              <dt>{t.expires}</dt>
              <dd>{formatDate(preview.expiresAt, locale)}</dd>
            </div>
            {Boolean(details.onlineRequired) && (
              <div>
                <dt>{t.targetStatus}</dt>
                <dd className={details.online ? 'success-text' : 'danger-text'}>
          ...
          <div className="review-actions">
            ...
            <button
              ...
              disabled={
                busy ||
                (preview.requiresTypedConfirmation &&
                  confirmation.trim().toLowerCase() !== 'execute')
              }
              onClick={() => void confirm()}
            >
              {busy ? <LoaderCircle className="spin" /> : <ShieldCheck />}
              {busy ? t.executing : t.confirmChange}
            </button>
          </div>
        </div>
      ) : (
        <form className="action-form" onSubmit={submitPreview}>
          <label>
            {t.selectAction}
            <select
              value={draft.type}
            ...
            >
              ...
              <option value="grant_balance">{t.grant}</option>
              <option value="debit_balance">{t.debit}</option>
          ...
          {draft.type !== 'reverse' && (
            <label>
              ...
              <input
                ...
                onChange={(event) => update('playerId', event.target.value)}
          ...
          {draft.type === 'set_role' && (
            <label>
              ...
              >
                ...
                <option value="economy_manager">economy_manager</option>
                <option value="content_manager">content_manager</option>
                <option value="owner">owner</option>
              </select>
            </label>
          )}
          {(draft.type === 'grant_balance' ||
            draft.type === 'debit_balance') && (
            <label>
              {t.amountLabel}
              <input
                required
          ...
          {draft.type === 'reverse' && (
            <>
              <label>
                ...
                <input
                  required
                  value={draft.originalTransactionId}
                  onChange={(event) =>
                    update('originalTransactionId', event.target.value)
                  }
                  placeholder="00000000-0000-0000-0000-000000000000"
                />
              </label>
              <label>
                ...
                <select
                  ...
                  onChange={(event) =>
                    update('compensation', event.target.value)
                  }
                >
                  <option value="none">none</option>
                  <option value="refund_without_items_or_stock">
                    refund_without_items_or_stock
                  </option>
                </select>
              </label>
            </>
          ...
          >
            {busy ? <LoaderCircle className="spin" /> : <ShieldCheck />}
            {busy ? t.reviewing : t.reviewChange}
          </button>
        </form>
      )}
    </Modal>
...
function HelpDialog({ t, onClose }: { t: Translator; onClose: () => void }) {
  const shortcuts = [
    ['Ctrl K /', t.focusSearch],
    ['Alt 1–6', t.switchSection],
    ['Esc', t.goBack],
    ['R', t.refreshData],
    ['?', t.openShortcutHelp],
  ];
  return (
    <Modal title={t.shortcutHelp} closeLabel={t.close} onClose={onClose}>
      <div className="help-content">
        ...
        <button className="pixel-button wide" data-autofocus onClick={onClose}>
          {t.close}
        </button>
      </div>
    </Modal>
  ...
  return (
    ...
    >
      <dialog
        ref={dialogRef}
        open
        className={`modal block-panel ${wide ? 'wide' : ''}`}
        aria-modal="true"
...
function PlayerCell({
  ...
  return (
    <button className="player-cell" onClick={onClick}>
      <span className="mini-avatar">
        {(name || '?').slice(0, 1).toUpperCase()}
      </span>
      <span>
        <strong>{name || shortId(id)}</strong>
...
function PlayerLink({
  ...
  if (id === '00000000-0000-0000-0000-000000000000')
    return <span>{name || 'SYSTEM'}</span>;
  return (
    <button className="table-link" onClick={() => onPlayer(id)}>
      {name || shortId(id)}
    </button>
  );
}
...
function RoleBadge({ role }: { role: string }) {
  return <span className={`role-badge ${role}`}>{humanize(role)}</span>;
}
...
function Detail({
  ...
  return (
    <div className="detail-item">
      <span>{label}</span>
      <strong className={gold ? 'gold-text' : ''}>{value}</strong>
    </div>
  );
}
...
function LocalePicker({
  ...
  return (
    <button
      type="button"
      className={compact ? 'locale-button compact' : 'locale-button'}
...
function ErrorState({
  ...
  return (
    <div className="error-state block-panel">
      <AlertTriangle />
      <h2>{message}</h2>
      <button className="pixel-button" onClick={retry}>
        <RefreshCw />
        {t.retry}
      </button>
    </div>
...
function formatNumber(value: string | number, locale: Locale): string {
  try {
    return BigInt(value).toLocaleString(localeTags[locale]);
  } catch {
    return String(value);
  }
}
const activityCopyKeys: Record<string, CopyKey> = {
  combat: 'activityCombat',
  cooking: 'activityCooking',
  mining: 'activityMining',
  exploration: 'activityExploration',
  hunting: 'activityHunting',
  building: 'activityBuilding',
  farming: 'activityFarming',
};
function activityLabel(track: string, t: Translator): string {
  const key = activityCopyKeys[track];
  return key ? t[key] : humanize(track);
}
function activityPercent(row: ActivityProgressRow): number {
  if (row.maximum) return 100;
  try {
    const required = BigInt(row.experienceForNextLevel);
    if (required <= 0n) return 0;
    return Math.max(
      0,
      Math.min(
        100,
        Number((BigInt(row.experienceIntoLevel) * 100n) / required),
      ),
    );
  } catch {
    return 0;
...
function humanize(value: string): string {
  return value.replace(/^rovenfall:/, '').replaceAll('_', ' ');
}
function errorMessage(reason: unknown, locale: Locale): string {
  if (reason instanceof ApiError) {
    ...
    if (reason.code === 'TARGET_OFFLINE')
      return locale === 'ko'
        ? '경제 거래를 되돌리려면 대상 사용자가 온라인이어야 합니다.'
        : 'The target player must be online to reverse an economy transaction.';
    return `${reason.message} (${reason.code})`;
  }
  return locale === 'ko'
    ? '서버 브리지에 연결할 수 없습니다. 서버 실행 상태와 포트를 확인하세요.'
    : 'Cannot reach the server bridge. Check the server and port.';
}
